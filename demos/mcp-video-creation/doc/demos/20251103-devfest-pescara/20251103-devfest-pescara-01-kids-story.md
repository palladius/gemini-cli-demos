...
```markdown
<!-- Remember to change the output folder below.-->
<!-- First show: /mcp list -->
...
- 🤖 Marvin l'androide paranoico (che sara' la mascotte di Alessandro e fara' da contrappeso tragicomico all'allegria e spensieratezza del bambino)
...
Ale parla Italiano e Inglese (e Tedesco).

Usa la cartella 📁 `out/pescara/demo01-alessandro-story-8nov-4REAL/`.
...
Ora crea un file sonoro per ciascuna lingua e dagli un nome appropriato (eg, story-it.wav, story-en.wav, ... ) COMINCIANDO con la parte italiana.
...
Nota che ho avuto problemi di timout su Chirp API a sintetizzare il TUTTO. Se fallisce, spezzalo in 2-4 parti (ma non di piu).
...
```markdown
<!-- 5. Create video (Veo) then .gif (ffmpeg) -->
...
Transform then the video into a GIF file.
```
...
```markdown
<!-- 6. merge all into README.md -->

Finally create a README.md which links the video, embeds the GIF, links ther audios, and points to the HTML and MarkDown files. basically, an index with all the links where the user can come
