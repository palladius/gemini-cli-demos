...
1. **Translate to French** and  **Generate Audio**:
...
*Notes*:
   - `say` is a native macOS tool (offline, no external APIs).
   - For Linux you might weant to use `espeak` instead. `espeak -v fr` should be able to speak French, but its very basic and metallic.
   - Make sure to speak it out loud for max effect in the demo
